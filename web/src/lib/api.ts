...
export const api = {
  ...
  slots: {
    list: (params: { from: string; to: string; serviceTypeId?: number; branchId?: number }) => {
      const q = new URLSearchParams({ from: params.from, to: params.to })
      if (params.serviceTypeId) q.set('serviceTypeId', String(params.serviceTypeId))
  ...
  appointments: {
    ...
    list: (params: { mechanicId: number; from: string; to: string }) => {
      const q = new URLSearchParams({
        mechanicId: String(params.mechanicId),
        from: params.from,
        to: params.to,
      })
    ...
    patchStatus: (id: number, status: AppointmentStatus) =>
      request(
        wrap(z.object({}).passthrough()),
